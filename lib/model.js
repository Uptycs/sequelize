...
Model.prototype.upsert = function (values, options) {
  ...
  return instance.hookValidate(options).bind(this).then(function () {
    ...
    if (createdAtAttr) {
      delete instance.dataValues[createdAtAttr];
    }
    ...
    instances.forEach((instance) => {
      for (var attr in self.rawAttributes) {
        if (self.rawAttributes[attr].field &&
            instance.dataValues[self.rawAttributes[attr].field] !== undefined &&
            self.rawAttributes[attr].field !== attr
        ) {
          instance.set(attr, instance.dataValues[self.rawAttributes[attr].field]);
          delete instance.dataValues[self.rawAttributes[attr].field];
        }
      }
    });
